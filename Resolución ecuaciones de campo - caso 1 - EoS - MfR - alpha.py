...
M_rel = []
alpha_list = [-0.001,-0.01,-0.05,-0.125,-0.2]
for alpha in alpha_list:    
    ...
    M_rel.append(M[len(r1)]/M[-1])
    print(M[len(r1)]/M[-1])
    
alpha_set = np.linspace(-0.2,0, 10000)
F = (1+2*alpha_set)**4

plt.figure()
plt.scatter(alpha_list, M_rel, label=r'$M_{f(R)}(r_b) / M_{f(R)}^\infty $',c='b',marker='+',s=120, linewidths=1.8)
plt.plot(alpha_set, F, label=r'$(1+2\alpha)^4$',c='r') 
plt.xlabel(r'$\alpha (km^2)$')
plt.legend(loc=0)
plt.show()
